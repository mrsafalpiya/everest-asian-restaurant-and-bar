...
```
index.html        review page that links the two candidates (noindex)
b/  c/            the two home-page directions: noren and hikifuda broadside
...
DESIGN.md         design system record for the two worlds
```
...
3. The site is served at the custom domain in `CNAME` (everest-restro.stge.me). The variants live at `/b/` and `/c/`.

All paths are relative, so the project works at any base path. Once a direction is chosen, move its three files to the root, change `../shared` to `shared` and `../assets` to `assets` in them, delete the other folder, and trim DESIGN.md to that world.
